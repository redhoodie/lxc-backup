...
COMMAND = %{lxc monitor #{LXC_REMOTE}: | grep --line-buffered fs_progress | head -%d | sed --regexp-extended "s/.*?'(.*?)'/\\1/g"}

def poll_lxc_list
  containers = `lxc list #{LXC_REMOTE}: | grep "#{LXC_CONTAINER_COPYING_TAG}"`.split("\n").collect{|container| container.gsub /^\| (.*?)-copying.*?$/, '\1'}
